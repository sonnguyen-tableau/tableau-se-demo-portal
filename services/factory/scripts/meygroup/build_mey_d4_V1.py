"""D4 Nhân Sự — VERSION 1 (Style B sparkline KPIs).

Applies the D1/D2 V1 best practice (skill tableau-exec-dashboard §10): a 6-month
trend sparkline under each of the 5 KPI hero numbers. HRMetrics has a real
`Month (HRMetrics)` dimension, so every sparkline is honest monthly data.

Surgical patch of the LIVE workbook. Publish as "Mey Group - Nhan Su V1".

Sparkline per KPI (recomputes the SAME calc grouped by month):
  Headcount   → AVG(Headcount)/month
  Quỹ lương   → SUM(PayrollActualVnd)/month
  Nghỉ việc   → AVG(AttritionRate)/month   (surfaces the Kinh-doanh T5-6 spike)
  KPI hoàn thành → AVG(KpiProgressPct)/month
  Tuyển mới   → SUM(NewHires)/month

Run:  uv run python scripts/meygroup/build_mey_d4_V1.py [--publish]
"""
import re
import sys
import uuid
...
LIVE_TWB = Path("/tmp/d4/d4.twb")
OUT_TWB = Path("/tmp/wb-mey-d4-V1.twb")
DS = "sqlproxy.052gxnr1oq3gkv13grj2s0u1xyqj"
BRAND = "#1b75bc"
DATE = "Month (HRMetrics)"   # HR monthly dimension

# KPI card name → its calc id (already in the datasource). The sparkline reuses it.
KPIS = [
    ("KPI Headcount",   "Calculation_0090040000000101"),
    ("KPI Quỹ lương",   "Calculation_0090040000000102"),
    ("KPI Nghỉ việc",   "Calculation_0090040000000103"),
    ("KPI Hoàn thành",  "Calculation_0090040000000104"),
    ("KPI Tuyển mới",   "Calculation_0090040000000105"),
]
...
def sparkline_worksheet(kpi: str, calc_id: str) -> str:
    """Minimal Line sparkline: cols = continuous MONTH(HR date), rows = the KPI
    calc; all axes/gridlines/labels hidden. Superstore 'KPI (Line)' idiom."""
...
def restructure_kpi_zone(dash_xml: str, kpi: str) -> str:
    """Wrap the KPI leaf in a vert-flow [number 72% / spark 28%], rounded card on
    the outer zone (D2 V1 §10 pattern). show-title='false' on the spark leaf."""
...
def patch(xml: str) -> str:
    ...
    # 2) Grow the KPI strip (relative weight h='16000' → 22000) + restructure cards.
    ...
    # 3) Register sparklines in <windows> (hidden) + dashboard viewpoints.
...
def build():
    ...
    print(f"D4 V1: {len(xml2):,} bytes  XML {validate(xml2)}")
    print(f"  sparklines: {[spark_name(n) for n,_ in KPIS]}")
    return xml2
...
def publish():
    ...
    twbx = Path("/tmp/wb-mey-d4-V1.twbx")
    with zipfile.ZipFile(twbx, "w", zipfile.ZIP_DEFLATED) as z:
        z.write(OUT_TWB, arcname="Mey Group - Nhan Su V1.twb")
    ...
    with server.auth.sign_in(auth):
        item = tsc.WorkbookItem(project_id="fab7f92a-6f33-4572-9e6f-7769017f58cf", name="Mey Group - Nhan Su V1", show_tabs=False)
